...
class CenterFlowLayout: NSCollectionViewFlowLayout {
  open override func layoutAttributesForElements(in rect: CGRect) -> [NSCollectionViewLayoutAttributes] {
   let layoutAttributesForElements = super.layoutAttributesForElements(in: rect)
    ...
    var representedElements: [NSCollectionViewLayoutAttributes] = []
    ...
    if scrollDirection == .vertical {
      for layoutAttributes in layoutAttributesForElements {
        ...
        let currentItemAttributes = layoutAttributes.copy() as! NSCollectionViewLayoutAttributes
      ...
      return representedElements + cells.flatMap { group -> [NSCollectionViewLayoutAttributes] in
...
extension NSCollectionViewFlowLayout {
  internal func evaluatedSectionInsetForSection(at section: Int) -> NSEdgeInsets {
    return (collectionView?.delegate as? NSCollectionViewDelegateFlowLayout)?.collectionView?(collectionView!, layout: self, insetForSectionAt: section) ?? sectionInset
  }
  internal func evaluatedMinimumInteritemSpacingForSection(at section: Int) -> CGFloat {
    return (collectionView?.delegate as? NSCollectionViewDelegateFlowLayout)?.collectionView?(collectionView!, layout: self, minimumInteritemSpacingForSectionAt: section) ?? minimumInteritemSpacing
  }
}
